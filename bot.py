...
async def city_choisen(message: types.Message, state = FSMContext):
...
async def user_city_weather(message: types.Message):
    ...
    if city is None:
        text = 'Пожалуйста установите свой город проживания'
        markup = types.reply_keyboard.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
...
async def user_city_choisen(message: types.Message, state = FSMContext):
    if message.text[0].islower():
        await message.answer('Названия городов пишутся с большой буквы')
        return
    ...
    text = f'Я запомнил {user_data.get("waiting_user_city")} Ваш город'
    await message.answer(text, reply_markup=markup)
    await state.finish()
    ...
    async with state.proxy() as data:
        ...
        if query_type == 'next':
            ...
            if data['current_page']*4 >= len(reports):
                ...
                inline_markup.row(
                    ...
                    types.InlineKeyboardButton(text=f'{data["current_page"]+1}/{total_pages}', 
                    callback_data='None')
                )
                await call.message.edit_text(text='История запросов:',reply_markup=inline_markup)
                return
        ...
        if query_type == 'prev':
            ...
            inline_markup.row(
                types.InlineKeyboardButton(text='Назад', callback_data=f'prev_{data["current_page"]}'),
                types.InlineKeyboardButton(text=f"{data['current_page']+1}/{total_pages}", callback_data='None'),
                types.InlineKeyboardButton(text='Вперед', callback_data=f'next_{data["current_page"]+2}')
            )
            await call.message.edit_text(text='История запросов:',reply_markup=inline_markup)
            ...
            for report in reports:
                if report.id == int(report_id):
                    inline_markup.add(
                        ...
                        types.InlineKeyboardButton(text ='Удалить отчет', callback_data=f'delete_report_{report_id}')
                    )
